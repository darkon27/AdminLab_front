...
export class TipoTrabajadorService {
  ...
  ListaBusqueda(filtro: any) {
    return this.config.getHttp().post(`${this.urlma}ListarTipoTrabajador`, filtro)
      .toPromise()
  ...
  mantenimiento(codigo: number, dto: any, token: string) {
    ...
    return this.config.getHttp().post(`${this.urlma}MantenimientoTipoTrabajador/` + codigo, dto, { headers: headers })
      .toPromise()
      .then(response => response)
      .catch(error => error)
  }
