...
export class TipoadmisionMantenimientoComponent
  ...
  implements OnInit
{
    ...
    Promise.all([p1]).then((resp) => {
      if (this.validarform == "NUEVO") {
        ...
        this.TipoAdmisionService.ListaTipoAdmision(this.filtro).then((res) => {
          ...
          if (
            res[0].FechaModificacion == null ||
            res[0].FechaModificacion == undefined
          ) {
            this.fechaModificacion = undefined;
          } else {
  ...
  async coreGuardar() {
    if (this.estaVacio(this.dto.AdmCodigo)) {
      this.messageShow("warn", "Advertencia", "Ingrese un código válido");
      return;
    }
    if (this.estaVacio(this.dto.AdmDescripcion)) {
      this.messageShow("warn", "Advertencia", "Ingrese un nombre válido");
      return;
    }
    if (this.estaVacio(this.dto.AdmEstado)) {
      this.messageShow("warn", "Advertencia", "Seleccione una estado válido");
      return;
    }
    if (this.validarform == "NUEVO") {
      this.bloquearPag = true;
      ...
      this.TipoAdmisionService.MantenimientoTipoAdmision(
        2,
        this.dto,
        this.getUsuarioToken()
      ).then((res) => {
        this.bloquearPag = false;
        if (res != null) {
          //console.log("registrado:", res);
          if (res.mensaje == "Ok") {
            ...
            this.mensajeController.resultado = res;
            this.mensajeController.componenteDestino.coreMensaje(
