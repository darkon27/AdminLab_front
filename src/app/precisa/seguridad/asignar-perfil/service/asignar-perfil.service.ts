...
export class AsignarPerfilService {
    ...
    mantenimientoPerfiles(codigo: number, data: any, token: string) {
        const headers = new HttpHeaders().set("Authorization", token)
        return this.config.getHttp().post(`${this.urlseg}MantenimientoUsuario/` + codigo, data, { headers: headers })
            .toPromise()
            .then(response => response)
            .catch(error => error)
    }
